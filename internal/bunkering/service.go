...
func (s *Service) Approve(ctx context.Context, actor domain.Actor, orderID, requestID string) error {
	return s.Store.WithCommittedPrelude(ctx, func(ctx context.Context, db *sql.DB) error {
		...
		var permit string
		if err := db.QueryRowContext(ctx, `SELECT status FROM safety_permits WHERE order_id=?`, orderID).Scan(&permit); err != nil {
			return err
		}
		if permit != "pending" {
			return fmt.Errorf("%w: permit", domain.ErrConflict)
		}
		if _, err := db.ExecContext(ctx, `UPDATE safety_permits SET status='approved', issued_by=? WHERE order_id=?`, actor.ID, orderID); err != nil {
			return err
		}
		if _, err := db.ExecContext(ctx, `UPDATE transfer_orders SET state='approved', version=version+1 WHERE id=? AND tenant_id=? AND state='planned'`, orderID, actor.TenantID); err != nil {
			return err
		}
		return nil
...
func (s *Service) CompleteStep(ctx context.Context, actor domain.Actor, orderID string, position int, requestID string) error {
	if s.Store.Hooks.FailAudit {
		if _, err := s.Store.DB.ExecContext(ctx, `UPDATE transfer_steps SET status='completed', confirmed_at=? WHERE order_id=? AND position=?`, storage.StringTime(time.Now()), orderID, position); err != nil {
			return err
		}
	}
	return s.Store.WithTx(ctx, func(tx *sql.Tx) error {
