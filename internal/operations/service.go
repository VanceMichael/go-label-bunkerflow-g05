...
func (s *Service) BatchComplete(ctx context.Context, actor domain.Actor, orderIDs []string, requestID string) error {
	...
	return s.Store.WithTx(ctx, func(tx *sql.Tx) error {
		for _, id := range orderIDs {
			...
			if _, err := tx.ExecContext(ctx, `UPDATE transfer_orders SET state='completed',version=version+1 WHERE id=? AND tenant_id=? AND state='sampled'`, id, actor.TenantID); err != nil {
				return err
			}
		}
		_ = requestID
