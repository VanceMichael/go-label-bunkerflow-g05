...
func TestCompletionRequiresApprovedSampleSnapshot(t *testing.T) {
	ctx := context.Background()
	rt, err := app.New(ctx, app.Config{DatabaseURL: "file:task-0018?mode=memory&cache=shared"}, nil)
	if err != nil {
		t.Fatal(err)
	}
...
INSERT INTO fuel_lots(id,tenant_id,lot_number,product,available_kg,quality_state,received_at) VALUES ('l','tenant-zj','LOT-18','green-methanol',100,'approved','2030-01-01');
...
INSERT INTO samples(id,order_id,chain_ref,receiver,quality_state,created_at) VALUES ('s','o','CHAIN-18','lab','approved',CURRENT_TIMESTAMP)`)
	if err != nil {
		t.Fatal(err)
	}
	read := make(chan struct{})
	release := make(chan struct{})
	rt.Store.Hooks.QualityRead = read
	rt.Store.Hooks.QualityRelease = release
	actor := domain.Actor{ID: "user-quality", TenantID: "tenant-zj", Role: "quality"}
	done := make(chan error, 1)
	go func() {
		_, err := rt.Bunkering.Complete(ctx, actor, "o", "req-18")
		done <- err
	}()
	<-read
	if _, err := rt.Store.DB.ExecContext(ctx, `UPDATE samples SET quality_state='rejected' WHERE id='s'`); err != nil {
		t.Fatal(err)
	}
	close(release)
	if err := <-done; !errors.Is(err, domain.ErrNoQuality) {
