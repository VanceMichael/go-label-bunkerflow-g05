...
func ClaimWindowUnchecked(ctx context.Context, tx *sql.Tx, id, tenantID, owner string) (sql.Result, error) {
	return tx.ExecContext(ctx, `UPDATE bunker_windows SET status='claimed', owner_id=?, version=version+1 WHERE id=? AND tenant_id=?`, owner, id, tenantID)
}
