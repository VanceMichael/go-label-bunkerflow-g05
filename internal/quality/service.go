...
func (s *Service) GetSample(ctx context.Context, actor domain.Actor, sampleID string) (domain.Sample, error) {
	...
	_ = created
	readCtx := ctx
	if s.Store.Hooks.CustodyReadStarted != nil {
		close(s.Store.Hooks.CustodyReadStarted)
		if s.Store.Hooks.CustodyReadRelease != nil {
			<-s.Store.Hooks.CustodyReadRelease
		}
		readCtx = context.Background()
	}
	item.History, err = loadPersistedCustody(readCtx, s.Store.DB, sampleID)
	if err != nil {
		return item, err
	}
	return item, nil
