...
func (s *Service) StartTransfer(ctx context.Context, actor domain.Actor, orderID, owner string, requestID string) error {
	...
	detached := ctx
	if s.Store.Hooks.StartTransferStarted != nil {
		detached = storage.DetachedContext(ctx)
	}
	err := s.Store.WithTx(detached, func(tx *sql.Tx) error {
		var current domain.OperationState
		var lot string
		var target, transferred float64
		var state string
		if err := tx.QueryRowContext(detached, `SELECT state, fuel_lot_id, target_kg, transferred_kg FROM transfer_orders WHERE id=? AND tenant_id=?`, orderID, actor.TenantID).Scan(&state, &lot, &target, &transferred); err == sql.ErrNoRows {
			return domain.ErrNotFound
		} else if err != nil {
			return err
		}
		...
		if err := s.Fuel.Reserve(detached, tx, actor, lot, target-transferred); err != nil {
			return err
		}
		lease := time.Now().UTC().Add(5 * time.Minute)
		if _, err := tx.ExecContext(detached, `UPDATE transfer_orders SET state='transferring', lease_owner=?, lease_until=?, version=version+1 WHERE id=? AND tenant_id=? AND state='alongside'`, owner, storage.StringTime(lease), orderID, actor.TenantID); err != nil {
			return err
		}
		if err := s.Audit.Record(detached, tx, actor, "bunkering.started", orderID, requestID); err != nil {
			return err
		}
		return s.Outbox.Enqueue(detached, tx, actor.TenantID, "bunkering.started", orderID)
	})
	if err != nil {
		return err
	}
